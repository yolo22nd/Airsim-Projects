...
async def main():
    global running, results, processed_image
    try:
        warm_up_model()
        ...
        # Takeoff
        print("Taking off...")
        client.takeoffAsync()  # Use synchronous takeoff method
        await asyncio.sleep(10)

        # Move to 15 meters height
        print("Moving to 5 meters altitude...")
        client.moveToPositionAsync(-5, 0, -3, 5)  # Use synchronous moveToPosition method
        await asyncio.sleep(10)

        # Monitor for person detection
        person_detected = False
        while running and not person_detected:
            with processed_image_lock:
                if latest_image is not None:
                    # Preprocess image
                    img = cv2.resize(latest_image, (640, 640))
                    ...
                    processed_image = latest_image.copy()
        ...
        if person_detected:
            print("Person detected! Hovering for 5 seconds...")
            time.sleep(5)  # Hover for 5 seconds

            # Increase altitude
            print("Increasing altitude...")
            client.moveToPositionAsync(-5, 0, -5, 5)  # Move to 20 meters altitude
        await asyncio.sleep(5)

        # Return to launch
        print("Returning to launch...")
        client.moveToPositionAsync(0, 0, -6, 5)
        await asyncio.sleep(5)
        client.landAsync()
        await asyncio.sleep(5)
